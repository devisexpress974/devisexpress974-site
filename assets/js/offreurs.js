...
function fillSelect(select, items){
  ...
  function render(items){
    ...
    try{
      const res = await window.DX_API.getAny(["listOffreursPublic","listOffreurs","getOffreursPublic"], {});
      ...
      ALL = [];
      LOADED = false;
      countBox.textContent = "Erreur de chargement.";
      return false;
    }
  }
  ...
  function showEmpty(msg){
    list.innerHTML = `
      <div style="background:#fff;border:1px solid rgba(0,0,0,0.08);border-radius:16px;padding:14px;">
        <div style="font-weight:900;">${esc(msg || "Choisis un service (obligatoire)")}</div>
        <div style="color:#666;font-weight:700;font-size:13px;margin-top:6px;">Les offreurs ne s’affichent qu’après sélection du domaine d’activité (pour garder la page rapide).</div>
      </div>
    `;
  }
  ...
  async function apply(){
    ...
    if(!LOADED){
      ...
      if(!ok){
        showEmpty("Impossible de charger les offreurs");
        return;
      }
    }
  ...
  async function reload(){
    const s = serviceFilter.value;
    if(!s){
      countBox.textContent = "Choisis d’abord un service.";
      showEmpty("Choisis un service (obligatoire)");
      return;
    }
    LOADED = false;
