// mur-demandes.js (v200) — Mur public "plié / déplié" + PJ + bouton "Voir plus"
document.addEventListener("DOMContentLoaded", () => {
  ...
  const COMMUNES_BY_ZONE = {
    ...
    "Est": ["Saint-André","Bras-Panon","Saint-Benoît","Sainte-Rose","Salazie","La Plaine-des-Palmistes"],
    "Sud": ["L'Étang-Salé","Les Avirons","Saint-Louis","Cilaos","Entre-Deux","Le Tampon","Saint-Pierre","Petite-Île","Saint-Joseph","Saint-Philippe"]
  };
  ...
  async function fetchMe(){
    ...
    try{
      ...
      if(res && res.ok && res.data) STATE.me = res.data;
      else STATE.me = null;
    }catch(e){
      STATE.me = null;
    }
  ...
  async function fetchServices(){
    try{
      ...
      if(serviceFilter){
        const labels = services.map(s => s.service_label).filter(Boolean);
  ...
  function render(){
    ...
    // bouton plus
    if(moreBtn){
      const canMore = STATE.loading ? false : (STATE.total === null ? true : (STATE.offset < STATE.total));
  ...
  function renderCard(d){
    ...
    const title = el("div", "dxPostTitle", (d.serviceAutre || d.service || "Demande"));
    const meta = el("div", "dxPostMeta", `${d.zone || "Sur toute l'île"}${d.commune ? " • " + d.commune : ""}${d.createdAt ? " • " + fmtDate(d.createdAt) : ""}`);
    top.appendChild(title);
    top.appendChild(meta);

    const excerpt = el("div", "dxPostExcerpt");
    excerpt.textContent = String(d.description || "").trim().slice(0, 150) + (String(d.description || "").length > 150 ? "…" : "");
    top.appendChild(excerpt);

    const actions = el("div", "dxPostActions");
    const btn = el("button", "dxPostToggle", "En voir plus");
    btn.type = "button";
  ...
  async function toggleCard(card, btn){
    ...
    exp.hidden = !opening;
    btn.textContent = opening ? "Voir moins" : "En voir plus";
    ...
    try{
      ...
      exp.innerHTML = "";
      exp.appendChild(el("div","dxNotice err", e?.message || "Erreur réseau."));
    }
  }
  ...
  function cardFindToggleFor(id){
    ...
    if(!card) return null;
    return card.querySelector(".dxPostToggle");
  }
  ...
  function maybeAutoOpen(filtered){
    ...
    if(card){
      ...
      const btn = card.querySelector(".dxPostToggle");
      if(btn){
        // open + scroll
        btn.click();
      ...
      // if we have more to load, try again later
      if(STATE.total === null || STATE.offset < STATE.total){
        // keep openTried false
      }else{
        STATE.openTried = true;
      }
    }
