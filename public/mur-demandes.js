...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function getServiceLabel(s){ return (s && (s.label || getServiceLabel(s) || s.name || s.title) || "").toString(); }
  ...
  async function fetchFirst(){
    ...
    const res = await window.DX_API.getAny(
      ...
      { offset: 0, limit: STATE.limit, service: (serviceFilter && serviceFilter.value)||"", zone: (zoneFilter&&zoneFilter.value)||"", commune:(communeFilter&&communeFilter.value)||"", q:(qFilter&&qFilter.value)||"" }
    );
  ...
  async function fetchMore(){
    ...
    const res = await window.DX_API.getAny(
      ((STATE.me && (STATE.me.offreurId || STATE.me.offreurID)) ? ["listDemandesForOffreur","listDemandesOffreur"] : ["listDemandesPublic","listDemandes","getDemandesPublic"]),
      { offset: STATE.offset, limit: STATE.limit, service: (serviceFilter && serviceFilter.value)||"", zone: (zoneFilter&&zoneFilter.value)||"", commune:(communeFilter&&communeFilter.value)||"", q:(qFilter&&qFilter.value)||"" }
    );
